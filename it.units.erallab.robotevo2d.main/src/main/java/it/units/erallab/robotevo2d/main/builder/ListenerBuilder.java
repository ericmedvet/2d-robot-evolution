...
public class ListenerBuilder {
  ...
  public static <G, S, Q> Function<Experiment<G, S, Q>, AccumulatorFactory<? super POSetPopulationState<G, S, Q>,
      List<File>, Run<?, ?>>> lastBestVideo(
    ...
    return experiment -> AccumulatorFactory.last((state, runParamMap) -> {
      List<File> files = new ArrayList<>();
      for (NamedTask<? super S> namedTask : namedTasks) {
        String fileName = fileNameTemplate.formatted(UUID.nameUUIDFromBytes(
            (namedTask.map().toString() + runParamMap.map().toString()).getBytes()
        ));
