...
public class Mappers {
    ...
    return InvertibleMapper.from(
        (supplier, x) -> () -> {
          ...
          return new DistributedNumGridVSR(
              new GridBody(
                  grid.map(
                      vt -> new GridBody.SensorizedElement(
                          new GridBody.Element(vt, Voxel.DEFAULT_MATERIAL),
                          sensors
                      )
                  )
              ),
              grid.map(vt -> vt.equals(GridBody.VoxelType.NONE) ? null : beforeM.mapperFor(ePair).apply(x).second()),
              nOfSignals,
              directional
          );
        },
  ...
  private static void checkIOSizeConsistency(NumMultiBrained numMultiBrained) {
    List<Integer> inSizes = numMultiBrained.brains()
        .stream()
        .map(NumericalDynamicalSystem::nOfInputs)
        .distinct()
        .toList();
    List<Integer> outSizes = numMultiBrained.brains()
        .stream()
  ...
  private static void checkNumericalParametrizedSizeConsistency(NumMultiBrained numMultiBrained) {
    List<Integer> brainSizes = numMultiBrained.brains()
        .stream()
        .map(
            b -> ((double[]) Composed.shallowest(b, NumericalParametrized.class)
                .orElseThrow()
                .getParams()).length
        )
        .distinct()
  ...
  public static <X, T extends NumMultiBrained> InvertibleMapper<X, Supplier<T>> dsToNpHeteroBrains(
    ...
    List<Integer> brainSizes = target.brains()
        .stream()
        .map(
            b -> ((double[]) Composed.shallowest(b, NumericalParametrized.class)
                .orElseThrow()
    ...
    return beforeM.andThen(
        InvertibleMapper.from(
            (supplier, values) -> {
              ...
              return () -> {
                @SuppressWarnings("unchecked") T t = (T) builder.build(
        ...
        InvertibleMapper.from(
            (supplier, values) -> {
              ...
              return () -> {
                ...
                t.brains()
                    .forEach(
                        b -> Composed.shallowest(b, NumericalParametrized.class)
                            .orElseThrow()
  ...
  public static <X> InvertibleMapper<X, Supplier<DistributedNumGridVSR>> ndsToFixedBodyHomoDistributedVSR(
    ...
    // check consistency
    List<Integer> inputSizes = body.grid()
        .entries()
        .stream()
        ...
        .distinct()
        .toList();
    List<Integer> outputSizes = body.grid()
        .entries()
        .stream()
        .filter(e -> !e.value().element().type().equals(GridBody.VoxelType.NONE))
    ...
    return InvertibleMapper.from(
        (supplier, x) -> () -> new DistributedNumGridVSR(
            ...
            body.grid()
                .map(
                    se -> se.element().type().equals(GridBody.VoxelType.NONE) ? null : beforeM.mapperFor(nds).apply(x)
                ),
            nOfSignals,
            directional
        ),
